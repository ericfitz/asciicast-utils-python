...
# /// script
# requires-python = ">=3.8"
# dependencies = ["websockets>=10.0,<12.0"]
# ///
        ...
        if self.event_loop and self.running:
            try:
                ...
                self.event_loop.call_soon_threadsafe(
                    self.broadcast_queue.put_nowait, (event_type, data)
                )
            except:
...
class AsciinemaRecorder:
    ...
    def write_header(self) -> None:
        ...
        if self.monitor_server:
            ...
            self.monitor_server.terminal_state.set_session_metadata(
                self.shell_command, self.output_file
            )
    ...
    def restore_terminal(self) -> None:
        ...
        if self.original_tty_settings:
            termios.tcsetattr(
                sys.stdin.fileno(), termios.TCSADRAIN, self.original_tty_settings
            )
        ...
        while True:
            try:
                ...
                ready_fds, _, _ = select.select(
                    [stdin_fd, master_fd, stderr_fd], [], [], 0.1
                )
                    ...
                    if fd == stdin_fd:
                        ...
                        try:
                            ...
                            if data:
                                ...
                                # Record input event
                                self.write_event(
                                    "i", data.decode("utf-8", errors="replace")
                                )
                        except OSError:
                            return
                        ...
                        try:
                            ...
                            if data:
                                ...
                                # Record output event
                                self.write_event(
                                    "o", data.decode("utf-8", errors="replace")
                                )
                            else:
                                # EOF from child
                                return
                        ...
                        try:
                            ...
                            if data:
                                ...
                                self.write_event(
                                    "e", data.decode("utf-8", errors="replace")
                                )
                            else:
...
Monitor examples:
  ...
  python record_session.py --monitor --monitor-host 0.0.0.0 --monitor-port 8888
    ...
    # Monitor options
    parser.add_argument(
        "--monitor", action="store_true", help="Enable terminal monitoring server"
    )
    ...
    if not os.path.isfile(shell_command) or not os.access(shell_command, os.X_OK):
        print(
            f"Error: Shell '{shell_command}' not found or not executable",
            file=sys.stderr,
        )
        sys.exit(1)
