...
### Input Consolidation (`consolidate_input.py`)

- **Merges scattered keystrokes** into `"c"` (command) records per typed command line
- **Non-destructive** - all original events preserved unchanged; `"c"` records are appended
- **Handles interleaved events** - accumulates input across intervening output/echo events
- **Flushes on line terminators** - Enter, newline, and Ctrl+C trigger consolidation
- **Raw character preservation** - backspace, escape sequences, and tab kept as-is
...
uv run consolidate_input.py -o consolidated.cast session.cast
...
#### Consolidated Command Events (`"c"`) - Added by `consolidate_input.py`

```json
[1.456, "c", "ls -la\r"]
```

**Purpose**: Generated by `consolidate_input.py` to provide a single record per typed command line. The timestamp is from the first keystroke. All original events are preserved; `"c"` records are appended non-destructively. Existing asciicast tools will ignore unknown event types.
